...
`);
export function getClaim(address) {
    return db.prepare("SELECT * FROM claims WHERE address = ?").get(address);
}
export function getStats() {
    const claimAgg = db.prepare("SELECT COALESCE(SUM(claim_count), 0) AS totalClaims, COUNT(*) AS uniqueAddresses FROM claims").get();
    return claimAgg;
}
...
export function insertClaimEvent(params) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(params.address, params.txid ?? null, params.ipHash, params.userAgent ?? null, params.amountXec, params.eventCode ?? null, params.rmzGateRequired ? 1 : 0, params.rmzGatePassed ? 1 : 0, params.createdAt, params.status, params.error ?? null);
}
